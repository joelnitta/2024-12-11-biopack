...
- Extensionsとは、Quartoの機能を拡張するコード

- プロジェクトごとにインストールが必要（Gitのsub repositoryみたいなもの）

- たくさんあります！ <https://quarto.org/docs/extensions/>

---

- `quarto add <gh-org>/<gh-repo>` で加える
  - 例えば `quarto add _extensions/schochastics/academicons`
  - 加えたコードをコミットするべし
...
## キーが分からないときは検索バーをスマートに使う

- <https://quarto.org/>から検索して、**使いたいフォーマット**のドキュメンテーションを見る

- 同じキー（例えば、`title`）でも、アウトプットのフォーマットによって使い方が異なる場合があります

---

![](images/quarto_website_search.png)

## CSS（とSCSS）を少し勉強するといい

- 特にスライドやウェブサイトの作成において重要です

- Fenced Divの使い方の理解も深まります

- おすすめ：Emil Hvitfeldtの[Slidecraft-101](https://emilhvitfeldt.com/project/slidecraft-101/)

## 色々なYAMLがある

- コードチャンクの中のYAML
- 文書（`Qmd`ファイル）のヘッダーのYAML
- プロジェクトの`_quarto.yml`
- 他にもあります・・
- 使いわけがちょっと難しい
...
- QuartoはRMarkdownの次世代バージョンです。
- Quartoの機能は非常に豊富で、デザインも洗練されています。
- 機能が多すぎて全貌が掴みにくいこともあります。
- どんな小さなプロジェクトでもいいので、とりあえず使ってみるのがおすすめです。